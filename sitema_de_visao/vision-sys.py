...
def save():
    ...
    global hmax, hmin, smax, smin, vmax, vmin, masks, name
    mask_value = [
...
def rst():
    ...
    global slider_hmax, slider_hmin, slider_smax, slider_smin, slider_vmax, slider_vmin
...
def update():
    ...
    global canvas_rgb, canvas_hsv, delay
    ...
    if ret:
        ...
        canvas_hsv.create_image(0, 0, image=photo_hsv, anchor=NW)
...
window = Tk()
window.title('SVC - RAS - ' + __version__)
...
''' Labels '''
label_RGB = Label(window, text='Rasteramento em tempo real')
label_Mask = Label(window, text='Visualizador de Máscaras')

''' Conteiners '''
canvas_rgb = Canvas(window, width=width * (resize_factor),
                    height=height * (resize_factor))

canvas_hsv = Canvas(window, width=width * (resize_factor),
                    height=height * (resize_factor))

''' H value sliders '''
slider_hmax = Scale(window, orient=HORIZONTAL, variable=hmax,
                    label='HMax', length=300, from_=0, to=255)

slider_hmax.set(255)

slider_hmin = Scale(window, orient=HORIZONTAL, variable=hmin,
                    label='HMin', length=300, from_=0, to=255)

''' S value sliders '''
slider_smax = Scale(window, orient=HORIZONTAL, variable=smax,
                    label='SMax', length=300, from_=0, to=255)

slider_smax.set(255)

slider_smin = Scale(window, orient=HORIZONTAL, variable=smin,
                    label='SMin', length=300, from_=0, to=255)

''' V value sliders '''
slider_vmax = Scale(window, orient=HORIZONTAL, variable=vmax,
                    label='VMax', length=300, from_=0, to=255)

slider_vmax.set(255)

slider_vmin = Scale(window, orient=HORIZONTAL, variable=vmin,
                    label='VMin', length=300, from_=0, to=255)

''' Misc '''
name_text_box = Entry(window, textvariable=name)
btn_save = Button(window, text='Salvar Máscara', command=save)
btn_rst = Button(window, text='Resetar Máscara', command=rst)
btn_calb = Button(window, text='Calibrar Origem', command=calibrate)
...
update()  # função loop para atualização das imagens
window.config(menu=menu_bar)  # função de configuração da barra de menus
window.mainloop()  # função loop principal da janela
